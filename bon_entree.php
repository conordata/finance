...
<?php 
  ...
  $bons_s = carnetBon::get_all_bon('sortie');
...
<head>
	...
	<title>GFI - Pro | Bon Entrée</title>
        ...
        <div class="mdk-header js-mdk-header bg-primary" data-fixed>
          <div class="mdk-header__content">
            <nav class="navbar navbar-expand-md bg-primary navbar-dark d-flex-none">
              ...
              <div class="page-title m-0">Bon d'entrée</div>
							...
							<div class="card card-body p-2">
								<div class="row row-projects">
									<div class="col">
										...
										<h5 class="mb-0 "><?=sprintf("%04d", count($bons)+count($bons_s)+1); ?></h5>
									</div>
									<div class="col">
										<i class="material-icons text-warning md-36">receipt</i>
										<div class="mb-1">Bon d'entrée</div>
									...
									<div class="col" disabled>
										<i class="material-icons text-warning md-36"></i>
										<div class="mb-1">-</div>
										<h5 class="mb-0"></h5>
									</div>
									<div class="col">
      			...
      			<div class="card card-body card-padded">
							...
							<form method="post" action="execute/bon.php" class="card-body">
								...
								<div class="row mb-3">
									<div class="col-sm-4">
										<div class="form-group">
											<input name="sign_au" type="text" class="form-control" placeholder="Organisation">
										</div>
									</div>
									<div class="col-sm-4">
										<div class="form-group">
...
<?php
	...
	if (isset($_POST['taux'])) {
		...
		database('','INSERT INTO taux (date_, taux) VALUES ("'.date("Y-m-d", time()).'", "'.$_POST['taux_dollar'].'")');
	...
	if (isset($_POST['new_fee'])) {
		...
		$ref_fee=database("r", "SELECT MAX(reference) FROM fees");
			
		if (count(database("r", "SELECT reference FROM fees"))==0) {
			$ref_fee="f01";
		} 
		else {
		...
		database('','INSERT INTO fees (title, reference) VALUES ("'.$_POST['n_frais'].'", "'.$ref_fee.'")');

		database('', 'ALTER TABLE register ADD '.$ref_fee.' INT(11) DEFAULT(0)');
		database('', 'ALTER TABLE class ADD '.$ref_fee.' INT(11) DEFAULT(0)');

		echo "<meta http-equiv='refresh' content='0'>";
	}

	if (isset($_POST['n_option'])) {
		database('','INSERT INTO section (nom_section, reference) VALUES ("'.$_POST['section'].'", "'.strtoupper($_POST['ref_section']).'")');
	...
	if (isset($_POST['n_classe'])) {
		...
		database('','INSERT INTO class (classe, section, sous_classe, reference) VALUES ("'.$_POST['classe'].'", "'.$_POST['option'].'", "'.$_POST['s_classe'].'", "'.$class_ref.'")');
	...
	if (isset($_POST['n_frais'])) {
		...
		$rqst='UPDATE class SET inscription="'.$_POST['n_inscription'].'", minerval="'.$_POST['n_minerval'].'"';

		while ($i<count($frais)) {

			$rqst=$rqst.', '.$frais[$i]['reference'].'="'.$_POST[$n_fee].'"';			
